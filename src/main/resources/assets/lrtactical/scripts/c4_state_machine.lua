...
function main_track_states.using_hold.transition(this, context, input)
    ...
    if (input == INPUT_PUT_AWAY and not (main_track_states.using_hold.throwing == 1) ) then
        run_put_away_animation(context)
        -- 丢枪后转到最终态
        return this.main_track_states.final
    elseif (input == "throw") then
        main_track_states.using_hold.throwing = 1
        local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
...
function M:initialize(context)
    ...
    main_track_states.using_hold.throwing = 0
end
